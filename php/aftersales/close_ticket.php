...
if($_POST['ticket_id'])
{
    $stmt = $con -> prepare('UPDATE projects_tickets SET status = "CLOSED", user = ? WHERE id = ?');
    $stmt -> bind_param('si', $_POST['user'], $_POST['ticket_id']);
    $stmt -> execute();
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        addNewLog($row['contact_id'], $_POST['user'], "Ticket afgerond: " . $row['title']);
