...
if(isset($_POST['quoteid']) && $_POST['quoteid']!= '') {
    ...
    if($stmt = $con ->prepare('UPDATE quotes SET quote_date = ?, account_id = ?, intro=?, reference = ?, factor = ?, rate = ?, inkoop = ?, materiaal_pdf = ?, arbeid_pdf = ? WHERE id = ?')){
        $date = date_create();
        $dt = $date->format("Y-m-d H:i:s");
        $stmt->bind_param('sssssssiii', $dt, $_POST['userid'], $_POST['intro'], $_POST['reference'], $_POST['factor'], $_POST['rate'], $_POST['inkoop'], $_POST['materiaal_pdf'], $_POST['arbeid_pdf'], $_POST['quoteid']);
        $stmt->execute();
    ...
    if ($stmt = $con->prepare('INSERT INTO quotes (contact_id, quote_date, account_id, reference, intro, factor, rate, inkoop, materiaal_pdf, arbeid_pdf) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')) {
        // We do not want to e  xpose passwords in our database, so hash the password and use password_verify when a user logs in.
        $date = date_create();
        $dt = $date->format("Y-m-d H:i:s");
        $stmt->bind_param('isisssssii', $_POST['contact_id'], $dt, $_POST['userid'], $_POST['reference'], $_POST['intro'], $_POST['factor'], $_POST['rate'], $_POST['inkoop'],  $_POST['materiaal_pdf'], $_POST['arbeid_pdf']);
        $stmt->execute();
    ...
    if ($stmt = $con->prepare('INSERT INTO quote_chapters (chapter_name, quote_id, default_chapter_id, sort_order) VALUES (?, ?, ?, ?)')) {
        ...
        if(isset($chapterData['line_data']))
        {
            for($jdex = 0; $jdex < count($chapterData['line_data']); $jdex ++)
                {
                    if($stmt = $con -> prepare('INSERT INTO quote_lines (line_title, line_descr, quanitty, unit, price, vat, subtotal, quote_id, chapter_id, sort_order, price_inc, line_am_option, line_total, arbeid, materiaal_total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')){
                        $line_data = $chapterData['line_data'][$jdex];
                        $stmt->bind_param('sssssssiiisssss', $line_data['line_title'], $line_data['line_descr'], $line_data['quanitty'], $line_data['unit'], $line_data['price'], $line_data['vat'], $line_data['subtotal'], $quote_id, $chapter_id, $jdex, $line_data['price_inc'], $line_data['line_am_option'], $line_data['line_total'], $line_data['arbeid'], $line_data['materiaal_total']  );
                        $stmt->execute();
