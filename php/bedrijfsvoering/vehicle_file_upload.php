...
if($_POST['vehicle_id'] && isset($_FILES['file']['name'])) {
    ...
    if(in_array(strtolower($fileEXE), $valid_extensions)) {
        ...
        if ($stmt = $con -> prepare('INSERT vehicles_file (vehicle_id, name, file_type, file_exe, uploaded_date, file_path, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)')) {
            $name = "";
            ...
            $stmt -> bind_param('isisssi', $_POST['vehicle_id'], $name, $file_type, $fileEXE, $dt, $file_path, $user_id);
            $stmt -> execute();
            $id = $stmt -> insert_id;
           ...
           move_uploaded_file($_FILES['file']['tmp_name'],$location);
            ...
            $stmt = $con -> prepare("SELECT VF.*, A.username FROM vehicles_file VF LEFT JOIN accounts A ON VF.user_id = A.id WHERE VF.id = ?");
