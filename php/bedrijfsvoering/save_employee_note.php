...
if($_POST['employee_id']) {
    if($stmt = $con -> prepare("INSERT INTO employees_notes (employee_id, created_at, data, user_id, file_exe, file_path) VALUES (?, ?, ?, ?, ?, ?)"))
    {
        $fileEXE = null;
        $filename = null;
        if(count($_FILES) > 0)
        ...
        $stmt -> bind_param('ississ', $_POST['employee_id'], $dtt, $_POST['data'], $_POST['user_id'], $fileEXE, $filename);
        $stmt -> execute();
        $id = $stmt -> insert_id;

        $stmt = $con -> prepare("SELECT VN.*, A.username FROM employees_notes VN LEFT JOIN accounts A ON VN.user_id = A.id WHERE VN.id = ?");
