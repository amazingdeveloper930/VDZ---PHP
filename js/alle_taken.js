...
function getTableTaskRowFromRawData(contactid, task_line, suppliers)
{
    var html = "<td>" + task_line['name'] + "</td>";
    if(task_line['status'] == 'COMPLETED' || task_line['status'] == 'SKIPPED')
        html += "<td><i class='material-icons'>person</i>" + task_line['username'] + "</td>";
    else
        html += "<td></td>";
    if(task_line['status'] == 'COMPLETED')
    {
        if(task_line['isspecial_task']){
            html += "<td>&euro;" + task_line['price_inc'] + "</td>";
            html += "<td>&euro;" + task_line['price'] + "</td>";
            html += "<td>" + convertDateFormat(task_line['betaaldatum']) + "</td>";
            html += "<td></td>";
        }
        else{//
            if(task_line['supplier'] == 'true')
                {
                    ...
                    for (var k = 0; k < suppliers.length; k++) {
                        if (suppliers[k]['id'] == task_line['supplier_id'])
                            html += "<td>" + suppliers[k]['name'] + "</td>";
                    }
                }
            else
                html += "<td></td>";    
            if(task_line['order_date'] == 'true')
                html += "<td>" + convertDateFormat(task_line['besteldatum']) + "</td>";
            else
                html += "<td></td>";
            if(task_line['supply_date'] == 'true')
                html += "<td>" + convertDateFormat(task_line['leverdatum']) + "</td>";
            else
                html += "<td></td>";
            if (task_line['special_jaarplanning'] == 'YES' && task_line['supply_date'] == 'true')
                html += "<td><i class='material-icons'>check_box</i></td>";
            else
                html += "<td></td>";
        }
    }   
    ...
    html += "<td>" + task_line['timer_widget'] + "</td>" ;
    html += "<td></td>";
    return html;
}
...
function initialProjectTaskPopup(contactid) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            {
                html += '<tr st_row='  + result['task_special'][index]['id'] + '>' + getRawHtmlFromSpeicalTaskInternal(result['task_special'][index]) + '</tr>'; 
            }
            ...
            html += '<tr class="project_s_task_input_field st_newrow">' +
                    ...
                    '<td><div onclick="saveSpecialTaskInternal(' + contactid + ')" class="actiebutton tooltipped" data-position="top" data-tooltip="Save"><i class="material-icons">save</i></div></td>'
            '</tr>';
            html += "</table></div>";
            $(".project-task-table-panel").append(html);
        ...
        // this is for projectplanning
            ...
            for (var jdex = result['task_list'].length - 1; jdex >= 0; jdex--) {
                ...
                html = getTableTaskRowFromRawData(contactid, task_line, suppliers);
                $("#table-alltaken tr.taskrow[contactrow=" + contactid + "][taskrow=" + task_line['id'] + "]").html(html);
            }
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result == 'Opdracht opgeslagen.') {
                ...
                {
                    $(".contacten-table tr[contactrow=" + contactid + "].contactrow td:nth-child(5)").html("<span class='text-red'>Nog niet gepland</span>");
                }
                else
                {
                    $(".contacten-table tr[contactrow=" + contactid + "].contactrow td:nth-child(5)").html(convertDateFormat(startdatum));
                }
        ...
        success: function(result) {
            if(result['message'] == 'Taak afgerond!')
            {
                ...
                if(result['completed_task'])
                {
                    ...
                    html = getTableTaskRowFromRawData(contactid, result['completed_task'], result['suppliers']);
                    $("#table-alltaken tr.taskrow[contactrow=" + contactid + "][taskrow=" + result['completed_task']['id'] + "]").html(html);
                }
                if(result['created_task'])
                {
                    ...
                    html = getTableTaskRowFromRawData(contactid, result['created_task'], result['suppliers']);
                    $("#table-alltaken tr.taskrow[contactrow=" + contactid + "][taskrow=" + result['created_task']['id'] + "]").html(html);
                }
                if(result['created_special_task'])
                {
                    ...
                    item.append(html);
                    item.addClass("line_processing");
                    html = getTableTaskRowFromRawData(contactid, result['created_special_task'], result['suppliers']);
                    $("#table-alltaken tr.taskrow[contactrow=" + contactid + "][taskrow=" + result['created_special_task']['id'] + "]").html(html);
                }
        ...
        success: function(result) {
            if(result['message'] == 'Taak overgeslagen!') {
                ...
                if(result['skipped_task'])
                {
                    ...
                    html = getTableTaskRowFromRawData(contactid, result['skipped_task'], result['suppliers']);
                    $("#table-alltaken tr.taskrow[contactrow=" + contactid + "][taskrow=" + result['skipped_task']['id'] + "]").html(html);
                }
                if(result['created_task'])
                {
                    var item = $(".project-task-table tr[task_id=" + result['created_task']['id'] + "].project_task");
        ...
        success: function(result) {
            if(result['message'] == 'Taak gereset!')
            {
                ...
                if(result['restarted_task'])
                {
                    ...
                    item.append(html);
                    item.addClass("line_processing");
                    html = getTableTaskRowFromRawData(contactid, result['restarted_task'], result['suppliers']);
                    $("#table-alltaken tr.taskrow[contactrow=" + contactid + "][taskrow=" + result['restarted_task']['id'] + "]").html(html);
                }
...
function getTableRawHtmlFromSpeicalTask(item)
{
    var html = '';
    ...
    '<td >' + (item.vat * 100) + '%</td>' +
    '<td>&euro;' + item.price_inc + '</td>' +
    '<td>' + item.option + '</td>' +
    '<td colspan="2">' + convertDateFormat(item.date) + '</td>' +
    '<td></td>';
    return html;
}
function saveSpecialTaskInternal(contactid, st_id = 0)
{
    var row_ele = '';
    if(st_id == 0)
        ...
        $.ajax({
			...
			success: function(result) {
                if(result.mode == 'CREATE')
                {
                    ...
                    $("#tab_chapter_meer .st_newrow .st_vat").formSelect();
                    var html = "<tr st_row=" + result['id'] + ">" + getRawHtmlFromSpeicalTaskInternal(result) + "</tr>";
                    $(html).insertBefore("#tab_chapter_meer .st_newrow");
                    html = getTableRawHtmlFromSpeicalTask(result);
                    html = "<tr class='taskrow staskrow tr-gray' contactrow='" + contactid + "' staskrow='" + result['id'] + "'>" + html + "</tr>";
                    var beforeitem = $("#table-alltaken tr.staskrow[contactrow=" + contactid + "]").last();
                    if(beforeitem.length == 0)
                    {
                        beforeitem = $("#table-alltaken tr.chapterrow[contactrow=" + contactid + "][chapterrow='special_tasks']")
                        beforeitem.removeClass('tr-hidden');
                    }
                    beforeitem.after(html);
                }
                if(result.mode == 'UPDATE')
                {
                    var html = getRawHtmlFromSpeicalTaskInternal(result);
                    $("#tab_chapter_meer tr[st_row=" + st_id + "]").html(html);
                    html = getTableRawHtmlFromSpeicalTask(result);
                    $("#table-alltaken tr.staskrow[contactrow=" + contactid + "][staskrow=" + result['id'] + "]").html(html);
                }
            },
            error: function(e1, e2, e3){
...
function getRawHtmlFromSpeicalTaskInternal(item)
{
    var html = '';
    ...
    '<td><div onclick="editSpecialTaskInternal(' + item.contact_id + ', ' + item.id + ')" class="actiebutton tooltipped" data-position="top" data-tooltip="Update"><i class="material-icons">create</i></div></td>';
    return html;
}

function getRawHtmlFromSpeicalTaskEditionInternal(item)
{
    var html = '';
    html = '<td><input placeholder="Omschrijving" value="' + item['text'] + '" class="st_text" type="text"/></td>' + 
            ...
            '<td><div onclick="saveSpecialTaskInternal(' + item['contact_id'] + ', ' + item['id'] + ')" class="actiebutton tooltipped" data-position="top" data-tooltip="Save"><i class="material-icons">save</i></div></td>';
    return html; 
}
function editSpecialTaskInternal(contact_id, st_id){
    var data = {
        st_id: st_id
    };
    $.ajax({
        ...
        success: function(result) {
            if(result){
                ...
                $(parent_ele).empty();
                var html = getRawHtmlFromSpeicalTaskEditionInternal(result);
...
$(".opdrachten-option input").on('change', function(){
    
    if(!($("#cb-opgeleverd").is(":checked")))
    {
        $("#projecten .contacten-table tr[data-opgeleverd='ja']").hide();
    }

   
    if(($("#cb-opgeleverd").is(":checked")))
    {
        $("#projecten .contacten-table tr[data-opgeleverd='ja']").show();
    }

    if(!($("#cb-gestart").is(":checked")))
    {
        $("#projecten .contacten-table tr[data-gestart='nee']").show();
    }

   
    if(($("#cb-gestart").is(":checked")))
    {
        $("#projecten .contacten-table tr[data-gestart='nee']").hide();
    }
...
function toggleHold(){
    ...
    {
        toggle_hold = false;
        var shownChapter = $("#s_showoption").val();
        if(shownChapter == 0)
        {
            $("#table-alltaken tr.chapterrow").show();
            $("#table-alltaken tr.taskrow").show();
        }
        else{
            $("#table-alltaken tr.chapterrow[chapterrow=" + shownChapter + "]").show();
            $("#table-alltaken tr.taskrow[chapterrow=" + shownChapter + "]").show();
        }
        
        $("#btn_hold span").html('Inklappen');
        $("#btn_hold i").html('arrow_upward');
    }
    else{
        toggle_hold = true;
        $("#table-alltaken tr.chapterrow").hide();
        $("#table-alltaken tr.taskrow").hide();
        $("#btn_hold span").html('Uitklappen');
        $("#btn_hold i").html('arrow_downward');
    }
}
$("#table-alltaken .contactrow").on('click', function(e){
    var contact_id = $(this).attr('contactrow');
    if($("#table-alltaken tr.contactrow[contactrow=" + contact_id + "]").attr('chaptershown') == 'true'){
        $("#table-alltaken tr.chapterrow[contactrow=" + contact_id + "]").hide();
        $("#table-alltaken tr.taskrow[contactrow=" + contact_id + "]").hide();
        $("#table-alltaken tr.contactrow[contactrow=" + contact_id + "]").attr('chaptershown', 'false');
    }
        ...
        if(shownChapter == 0)
        {
            $("#table-alltaken tr.chapterrow[contactrow=" + contact_id + "]").show();
            $("#table-alltaken tr.taskrow[contactrow=" + contact_id + "]").show();
        }
        else{
            $("#table-alltaken tr.chapterrow[contactrow=" + contact_id + "][chapterrow=" + shownChapter + "]").show();
            $("#table-alltaken tr.taskrow[contactrow=" + contact_id + "][chapterrow=" + shownChapter + "]").show();
        }

        
        $("#table-alltaken tr.contactrow[contactrow=" + contact_id + "]").attr('chaptershown', 'true');
    }
