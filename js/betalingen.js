...
function updateContactInvoiceData(contact_id)
{
    $.ajax({
        type: "POST",
        url: "../php/opdracht/get_contact_invoice_data.php",
        data: {
            contact_id: contact_id
        },
        dataType: "json",
        success: function(result) {
            if(result)
            {
                var unpaid_invoice_count = 0;
                for(var index = 0; index < result.length; index ++)
                {
                    $(".contacten-table tr[contactid=" + contact_id + "][invoice_payment_task_id=" + result[index].invoice_payment_task_id + "] td:nth-child(2)").html(convertDateFormat(result[index].invoice_date));
                    $(".contacten-table tr[contactid=" + contact_id + "][invoice_payment_task_id=" + result[index].invoice_payment_task_id + "] td:nth-child(4)").html(result[index].invoice_price);
                    $(".contacten-table tr[contactid=" + contact_id + "][invoice_payment_task_id=" + result[index].invoice_payment_task_id + "] td:nth-child(3)").html(convertDateFormat(result[index].paid_date));
                    $(".contacten-table tr[contactid=" + contact_id + "][invoice_payment_task_id=" + result[index].invoice_payment_task_id + "] td:nth-child(5)").html(result[index].paid_price);
                    $(".contacten-table tr[contactid=" + contact_id + "][invoice_payment_task_id=" + result[index].invoice_payment_task_id + "] td:nth-child(6)").html(result[index].timer_widget);
                    if(result[index].invoice_price != null && result[index].invoice_price != result[index].paid_price)
                    unpaid_invoice_count ++;
                }
                if(unpaid_invoice_count == 0)
                {
                    $(".contacten-table tr[contactrow=" + contact_id + "]").removeClass('contract-unpaid');
                    $(".contacten-table tr[contactrow=" + contact_id + "]").addClass('contract-paid');
                }else{
                    $(".contacten-table tr[contactrow=" + contact_id + "]").removeClass('contract-paid');
                    $(".contacten-table tr[contactrow=" + contact_id + "]").addClass('contract-unpaid');
                }
            }
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result == 'Opdracht opgeslagen.') {
                ...
                initialProjectTaskPopup(contactid);
                updateContactInvoiceData(contactid);
                ...
                $(".contacten-table tr[contactrow=" + contactid + "]").attr('data-opgeleverd', plaatsing);
...
function toggleHold(){
    ...
    {
        toggle_hold = false;
        $("#table-betalingen tr.row-hold").show();
        $("#btn_hold span").html('Inklappen');
        $("#btn_hold i").html('arrow_upward');
    }
    else{
        toggle_hold = true;
        $("#table-betalingen tr.row-hold").hide();
        $("#btn_hold span").html('Uitklappen');
        $("#btn_hold i").html('arrow_downward');
    }
}
$("#table-betalingen .contactrow").on('click', function(e){
    var contact_id = $(this).attr('contactrow');

    if($("#table-betalingen tr.row-invoice-header[contactid=" + contact_id + "]").is(':visible'))
        $("#table-betalingen tr.row-hold[contactid=" + contact_id + "]").hide();
    else{
        $("#table-betalingen tr.row-hold[contactid=" + contact_id + "]").show();
    }
    
})


function savefactureren(contact_id, invoice_task_id)
{
    var facturenren = $("#table-betalingen tr[contactid=" + contact_id +"][invoice_payment_task_id=" + invoice_task_id + "] .cb-facturenren").prop('checked');
    if(facturenren == true)
        facturenren = 1;
    else 
        facturenren = 0;
    $.ajax({
        type: "POST",
        url: "../php/opdracht/save_factureren.php",
        data: {
            'contact_id': contact_id,
            'invoice_task_id' : invoice_task_id,
            'facturenren' : facturenren
        },
        dataType: "html",
        success: function(result) {
            //alert(result);
            if (result == 'Contact saved.') {
