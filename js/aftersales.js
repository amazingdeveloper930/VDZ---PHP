...
function updateTimerWiget(contactid)
{
    $.ajax({
        type: "POST",
        url: "../php/aftersales/get_project.php",
        data: {
            contactid: contactid
        },
        dataType: "json",
        success: function(result) {
            if(result.length > 0){
                ...
                var timer_widget = result.timer_4;
                if(result.m_status5 == 'NO')
                    timer_widget = result.timer_5;
                else if(result.m_status6 == 'NO')
                    timer_widget = result.timer_6;
                else if(result.m_status7 == 'NO')
                    timer_widget = result.timer_7;
                else if(result.m_status5 == 'YES')
                    timer_widget = result.timer_5;
                else if(result.m_status6 == 'YES')
                    timer_widget = result.timer_6;
                else if(result.m_status7 == 'YES')
                    timer_widget = result.timer_7;
                ...
                for(var index = 4; index <= 7; index ++){
                    if(result['m_status' + index] == 'YES' || result['m_status' + index] == 'SKIP')
                    {
                        var btn_restart = '<div class="actiebutton btn-refresh" onclick="restartProject(' + contactid + ', \'MODE' + index + '\')"><i class="material-icons">refresh</i></div>';
                        result['timer_' + index] += btn_restart;
                        $(".contacten-table tr[contactrow=" + contactid + "] td:nth-child(" + (index + 1) + ")").html(result['timer_' + index]);
                    }
                    if(result['m_status' + index] == 'NO')
                    {
                        var btn_done = '<span class="button btn btn-complete" onclick="completeProject(' + contactid + ', \'MODE' + index + '\')"><i class="material-icons">done</i></span>';
                        var btn_skip = '<span class="button btn btn-skip" onclick="skipProject(' + contactid + ', \'MODE' + index + '\')"><i class="material-icons">close</i></span>';
                        result['timer_' + index] += btn_done + btn_skip;
                        $(".contacten-table tr[contactrow=" + contactid + "] td:nth-child(" + (index + 1) + ")").html(result['timer_' + index]);
                    }
                }
            }
...
function completeProject(contact_id, mode = 'MODE4')
{
    var data = {
    ...
    $.ajax({
        type: "POST",
        url: "../php/aftersales/complete_project.php",
        data: data,
        dataType: "json",
        success: function(result) {
            if(result['message'] == 'Project opgeslagen!')
            {
                melding(result['message'], 'groen');
...
function skipProject(contact_id, mode = 'MODE4')
{
    var data = {
        contact_id: contact_id,
        mode: mode
    ...
    $.ajax({
        type: "POST",
        url: "../php/aftersales/skip_project.php",
        data: data,
        dataType: "json",
        success: function(result) {
            if(result['message'] == 'Project opgeslagen!')
...
function restartProject(contact_id, mode = 'MODE4')
{
    var data = {
        contact_id: contact_id,
        mode: mode
    ...
    $.ajax({
        type: "POST",
        url: "../php/aftersales/restart_project.php",
        data: data,
        dataType: "json",
        success: function(result) {
            if(result['message'] == 'Project gereset!')
            {
                melding(result['message'], 'groen');
                updateTimerWiget(contact_id);
        ...
        success: function(result) {
            if(result['message'] == 'Taak afgerond!')
            {
                ...
                if(result['completed_task'])
                {
                    ...
                    $(".contacten-table tr[contactrow=" + contactid + "] td:nth-child(7)").text(result['completed_task']['name']);
                }
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result == 'Opdracht opgeslagen.') {
                ...
                {
                    $(".contacten-table tr[contactrow=" + contactid + "] td:nth-child(6)").html("<span class='text-red'>Nog niet gepland</span>");
                }
                else
                {
                    $(".contacten-table tr[contactrow=" + contactid + "] td:nth-child(6)").html(convertDateFormat(startdatum));
                }
                var item = $("tr[contactrow=" + contactid + "]");
                if(plaatsing == 'nee')
                {
                    if($("#inactieveopdracbten tr[contactrow=" + contactid + "]").length > 0)
                    {
                        item.detach().appendTo("#actieveopdracbten table tbody");
                    }
                }

                if(plaatsing == 'ja')
                {
                    if($("#actieveopdracbten tr[contactrow=" + contactid + "]").length > 0)
                    {
                        item.detach().appendTo("#inactieveopdracbten table tbody");
                    }
                }
...
function closeTableTicket(ticket_id)
{
			...
			success: function(result) {
                if(result['message'] == 'Ticket sluiten.')
                {
                    ...
                    if(result['item'].length > 0)
                    {
                        ...
                        $(" .project_ticket[ticket_row=" + item['id'] + "]").empty();
                        var html = getTableTicketRawHtmlFromItem(item, 'normal', false);

                        $(" .project_ticket[ticket_row=" + item['id'] + "]").html(html);
                        if(item['status'] == 'CLOSED')
                            {
                                $(" .project_ticket[ticket_row=" + item['id'] + "]").addClass("ticket_closed");
                            $(" .project_ticket[ticket_row=" + item['id'] + "]").removeClass("ticket_opened");
                            }
                        else{
                            $(" .project_ticket[ticket_row=" + item['id'] + "]").removeClass("ticket_closed");
                            $(" .project_ticket[ticket_row=" + item['id'] + "]").addClass("ticket_opened");
                        }
                    }
                }
                else{
...
function restartTableTicket(ticket_id)
{
    $.ajax({
        type: "POST",
...
function getTableTicketRawHtmlFromItem(item, mode='normal', header = true)
{
    var html = "";
    if(mode == 'normal')
    {
    ...
    html += "<td><span class='button btn btn-sluten' onclick='closeTableTicket(" + item['id'] + ")' " + (item['status'] == 'OPENED' ? '':' disabled ') + "><i class='material-icons'>done</i> Sluiten</span>";
    if(item['status'] == 'CLOSED')
    ...
    else if(mode == 'edit')
    {
        ...
        html += "<td><input type='text' value='" + (item['bedrag_open']? item['bedrag_open'] : '') + "' class='txt_bedrag_open' onchange='convertNumber(this)'/></td>";
    }


    html += '<td><div class="actiebutton btn-ticket-refresh" onclick="restartTableTicket(' + item['id'] + ')"><i class="material-icons">refresh</i></div></td>';
    html += "<td>" + item['timer_widget'] + "</td>" + 
    "<td>";

    html += "<div class='actiebutton tooltipped'  data-position='top' onclick='gotoWeekplanning(" + item['contact_id'] + "," + item['id'] + ")'><i class='material-icons'>build</i></div>";
    html += "<div class='actiebutton tooltipped'  data-position='top' onclick='manageTickets(" + item['contact_id'] + ")'><i class='material-icons'>report_problem</i></div>";
    if(mode == 'normal')
    {
     html += "<div class='actiebutton' onclick='editTableTicket(" + item['id'] + ")'><i class='material-icons'>edit</i></div>" ;
    }
    else if(mode == 'edit')
    {
        html += "<div class='actiebutton' onclick='updateTableTicket(" + item['id'] + ")'><i class='material-icons'>save</i></div>" ;
    }
...
function updateTableTicketRowHtml(ticket_id, mode)
{
    $.ajax({
        type: "POST",
        ...
        success: function(result) {
            ...
            for(var index = 0; index < result.length; index ++)
            {
                $(".project-ticket-table tbody .project_ticket[ticket_row=" + result[index]['id'] + "]").html(getTableTicketRawHtmlFromItem(result[index], mode , false));
            }
            initDatePicker();
        }
    });
...
function editTableTicket(ticket_id)
{
    updateTableTicketRowHtml(ticket_id, 'edit');
    
}

function updateTableTicket(ticket_id)
{
    var container = ".project-ticket-table tbody .project_ticket[ticket_row=" + ticket_id + "]";
    var data = {
        ticket_id : ticket_id,
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            {
                melding(result['message'], 'groen');
                updateTableTicketRowHtml(ticket_id, 'normal');
            }
            else{
                melding(result['message'], 'rood');
            }
