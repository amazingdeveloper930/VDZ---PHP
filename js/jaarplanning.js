var quarter_list = ['KW1', 'KW2', 'KW3', 'KW4'];

var displayed_year = new Date().getFullYear();
var displayed_quarter = Math.ceil((new Date().getMonth()) / 4) + 1;

if($("#num_year").val())
{
    displayed_year = $("#num_year").val();
    displayed_quarter = $("#num_quarter").val();
}
function gotoPrevQuarter()
{
    displayed_quarter --;
    if(displayed_quarter == 0)
        {
            displayed_quarter = 4;
            displayed_year --;
        }
...
{
    $(".td-week").empty();  
    $(".td-jaarplanning-text").empty(); 
    $(".td-week").attr('year', displayed_year);
    $(".calender-quarter-header-txt").text(quarter_list[displayed_quarter - 1] + ", " + displayed_year);
    for(var jdex = 1; jdex <= 13; jdex ++)
    {
        $("#week-0-" + jdex + " .week-number").text("Week " + (jdex + (displayed_quarter - 1) * 13));
    }
    var data = {
        year : displayed_year,
        quarter : displayed_quarter
    };
    $.ajax({
        type: "POST",
        url: "../php/productie/get_project_list_quarter.php",
        data: data,
        dataType: "json",
        success: function(result) {
           if(result)
           {
               ...
               for(var index = 0; index < result.length; index ++){
                    if(addItem(result[index], jdex))
                        jdex ++;
               }
               
               $('.tooltipped').tooltip();

               for(var index = 0; index < $("#jaarplanning .td-week-header").length; index ++)
               {
                   var employee_count = employees[index];
                   
                   var needed_employee_count = 0;
                   for(var jdex = 0; jdex < $("#jaarplanning tbody tr td:nth-child(" + (index + 2) + ") .jaar-panel").length; jdex ++)
                   {
                    needed_employee_count += parseInt($($("#jaarplanning tbody tr td:nth-child(" + (index + 2) + ") .jaar-panel")[jdex]).text());
                   }
                   $("#jaarplanning #week-0-" + (index + 1) + " .week-employee-info").text(employee_count + " / " + needed_employee_count);
                   if(Math.abs(employee_count - needed_employee_count) >= 3)
                        $($("#jaarplanning .td-week-header")[index]).addClass('td-week-header-red');
                   else
                        $($("#jaarplanning .td-week-header")[index]).removeClass('td-week-header-red');
               }

               for(var jdex = 1; jdex < $("#jaarplanning_table tr").length; jdex ++ )
               {
                   if($($("#jaarplanning_table tr")[jdex]).find(".td-jaarplanning-text").text() == "")
                   {
                    $($("#jaarplanning_table tr")[jdex]).hide();
                   }
               }
           }
        },
...
function addDotItem(jaar_item, index){
    var date = jaar_item['leverdatum'];
    var quarter = (new Date(date).getMonth()) / 4 + 1;
    var year = new Date(date).getFullYear();
    var week = new Date(date).getWeek();
    
    if(!(year == displayed_year || ((year == displayed_year - 1) && displayed_quarter == 4)) )
        return false;
    week = week - (displayed_quarter - 1) * 13;
    if(week > 0 && week <= 13)
    {
        var class_str='';
        if($("#week-" + (index + 1) + "-" + week).find('.jaar-panel').length > 0)
            class_str='on-jaar-panel';
        var html = '<i class="material-icons tooltipped spec-jaar day-' + convertDateFormat(date) + ' ' + class_str + '" task_name="' + jaar_item['name'] + '" task_date="' + jaar_item['leverdatum'] + '"  data-position="top" data-tooltip="' + jaar_item['name'] + '<br/>' + convertDateFormat(jaar_item['leverdatum']) + '">circle</i>';
        $("#week-" + (index + 1) + "-" + week).append(html);
    }
    else{
        return false;
    }
...
{
    var date = item['startdatum'];
    var year = new Date(date).getFullYear();
    var week = new Date(date).getWeek();
    if(item['plan']!= null && item['plan'] != undefined && item['plan'].length > 0)
    {
        var flag = false;
        for(var jdex = 0; jdex < item['plan'].length; jdex ++)
        {
            var week_count = item['plan'][jdex]['week'];
            for(var k = 0; k < week_count; k ++)
            {
                year = year + Math.floor((week - 1) / 52);
                week = week - 52 * Math.floor((week - 1) / 52);
                var temp_quarter = Math.floor((week - 1) / 13) + 1;
                var temp_week = week - 13 * (temp_quarter - 1);
                
                if((temp_quarter == displayed_quarter) && (year == displayed_year))
                {
                    var temp_item = $("#week-" + (index + 1) + "-" + temp_week);
                    temp_item.append("<div class='tooltipped jaar-panel jaar-panel-" + item['plan'][jdex]['color'] + "' data-tooltip='" + item['plan'][jdex]['name']+"'>" + item['plan'][jdex]['medewerkers'] + "</div>");
                    flag = true;
                }
                else{
        ...
        if(flag){
            for(var jdex = 0; jdex < item['special_jaars'].length; jdex ++)
            {
                addDotItem(item['special_jaars'][jdex], index);
            }
            var  text_item = "<span class='prtext'>" + item['address'] + ", " +item['city'] + "</span>";
            $("#jaarplanning-text-" + (index + 1)).append(text_item);
             $("#jaarplanning-text-" + (index + 1)).attr('onclick', 'manageProjectTask(' + item['contact_id'] + ')');
        }
...
function showSpecialJaar(ele)
{
    showConfirm(convertDateFormat($(ele).attr('task_date')) + '<br/>' + $(ele).attr('task_name'), 'OK!', 'green', "closeConfirm()", false, ['waves-effect', 'waves-light', 'btn']);
}
displayQuarterData();
    ...
    $.ajax({
        ...
        success: function(result) {
            if(result){
                ...
                $(".contacten-table tr[contactrow=" + contactid + "] td:nth-child(8)").html(((timer_widget) ? timer_widget : ''));
            }
        },
...
function print_jaarplanning()
{
    var root_path = $("#root_path").val();
    var year = displayed_year;
    var quarter = displayed_quarter;
    var link = root_path + "print_jaarplanning?year="+year + "&quarter=" + quarter;
    location.href = link;
}