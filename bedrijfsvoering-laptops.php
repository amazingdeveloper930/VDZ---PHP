...
$stmt = $con -> prepare('SELECT L.*, E.name AS employee_name FROM laptop L LEFT JOIN employees E ON L.employee = E.id ORDER BY L.id');
$stmt -> execute();
	...
	<body class="app">
		...
		<div class="appcontent">
			...
			<div class="titlebar">
						...
						<div class="row">
							<div class="col s12">
								...
								<?php 
                                    ...
                                    <li class="tab col"><a class="active" href="#">Laptops + telefoons</a></li>
				...
				<span class="titlebarbutton button waves-effect waves-light btn" onclick="addNewLaptop()"><i class="material-icons">add</i> Toevoegen</span>
				<div style="clear:both"></div>
			</div>			

			<div id="laptop-panel" class="tab-content active">			
				<table class="laptop-table full-w-table sort-table">
					<thead>
					<tr>
						<th>Soort</th>
						<th>Merk</th>
						...
						<th>Abonnement tot</th>
						<th>Abonnement provider</th>
						<th>Maandprijs</th>
					...
					<?php if ($result) : while ($row = $result->fetch_assoc()) : 
						...
						<tr laptoprow="<?=$row['id'];?>">
							...
							<td><?=$row['merk'];?></td>
							<td><?=$row['type'];?></td>
							<td><?=dateFormat($row['aanschafdatum']);?></td>
							<td><?=$row['employee_name'];?></td>
							<td><?=dateFormat($row['abonnement_tot']);?></td>
							<td><?=$row['abonnement_provider'];?></td>
							<td>&euro;<?=$row['maandprijs'];?></td>

							<td>
								<div onclick="editLaptop(<?=$row['id'];?>)" class="actiebutton tooltipped" data-position="top" data-tooltip="Wijzigen"><i class="material-icons">edit</i></div>
				
								<div onclick="deleteLaptop(<?=$row['id'];?>)" class="actiebutton tooltipped" data-position="top" data-tooltip="Verwijderen"><i class="material-icons">delete</i></div>
							</td>
						</tr>	
